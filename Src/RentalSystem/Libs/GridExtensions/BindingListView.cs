        ...
        private void InitializeFiltering()
        {
            ...
            foreach (PropertyDescriptor property in _properties)
            {
                        ...
                        && property.PropertyType.GetGenericTypeDefinition().Equals(typeof(Nullable<>)))
                {
                    ...
                    _filterTable.Columns.Add(property.Name, underlyingType);
                }
                else
                {
            ...
            set
            {
                ...
                if (value.Length > 0)
                {
                    ...
                    for (int i = 0; i < Count; i++)
                    {
                        ...
                        {
                            if (_properties[j].PropertyType.IsGenericType
                                    && _properties[j].PropertyType.GetGenericTypeDefinition().Equals(typeof(Nullable<>))
                                            && _properties[j].GetValue(item) == null)
                            {
                                NullableConverter nullableConverter = new NullableConverter(_properties[j].PropertyType);
                                Type underlyingType = nullableConverter.UnderlyingType;
                                row[j] = Activator.CreateInstance(underlyingType);
                            }
                            else
                            {
