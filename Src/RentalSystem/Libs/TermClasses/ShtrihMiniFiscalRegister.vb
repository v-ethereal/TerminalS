...
''' <summary>
''' Класс для работы с фискальным регистратором Штрих-Мини-ФР.
''' Реализует интерфейс ITerminalFiscalRegister для использования в терминале оплаты.
''' </summary>
Public Class ShtrihMiniFiscalRegister
    Implements ITerminalFiscalRegister

    ''' <summary>
    ''' Структура для хранения информации о выравнивании текста или штрихкода при печати
    ''' </summary>
    ...
    ''' <summary>
    ''' Структура для хранения информации о типе штрихкода при печати
    ''' </summary>
    ...
    ''' <summary>
    ''' Структура для хранения информации о типе шрифта при печати
    ''' </summary>
    ...
    Friend Shared cfgDebugLevel As Integer = 1      '0 - нет логирования, 1 - только ошибки, 2 - подробное логирование
    Friend Shared cfgComPort As String = "COM3"     'Порт для подключения к ККМ
    Friend Shared cfgBaudRate As Integer = 115200   'Скорость обмена данными с ККМ, может быть 2400/4800/9600/19200/38400/57600/115200
    Friend Shared cfgReadTimeOut As Integer = 100   'Таймаут чтения данных от ККМ в миллисекундах
    Private FR As DrvFRLib.DrvFR                    'Объект драйвера для работы с ККМ
    ...
    ''' <summary>
    ''' Закрывает соединение с ККМ
    ''' </summary>
    Public Sub Disconnect()
        If FR IsNot Nothing Then
            Try
                If cfgDebugLevel >= 2 Then WriteLog("Method Disconnect is starting")
                FR.Disconnect()
                If FR.ResultCode = 0 Then
                    If cfgDebugLevel >= 2 Then WriteLog("Method Disconnect done")
                Else
                    WriteLog("Method Disconnect failed with RC = " & FR.ResultCode & vbCrLf & FR.ResultCodeDescription)
                End If
            Catch ex As Exception
                WriteLog("Exception during Disconnect: " & ex.Message)
            End Try
        End If
    End Sub

    ''' <summary>
    ''' Конструктор класса, выполняющий инициализацию и подключение к ККМ, а также проверку его состояния,
    ''' чтение параметров из конфигурационного файла TermClasses.cfg.
    ''' Если файл не найден, используются значения по умолчанию.
    ''' </summary>
    Public Sub New()
        MyBase.New()

        Dim ComPortNum As Integer = 3
        Dim BaudRateCode As Integer = 6
        Do
            ...
            Else
                ...
                Else
                    ...
                    Throw New ApplicationException("Invalid value " & cfgComPort & " for ComPortName parameter in configuration file")
                End If
            End If
                ...
                If IsNumeric(watr.Value) Then
                    ...
                    Select Case cfgBaudRate
                        ...
                        Case Else
                            ...
                            Throw New ApplicationException("Invalid value " & cfgBaudRate & " for ComPortBaudRate parameter in configuration file")
                    End Select
                Else
        ...
        If ComPortNum = 13 Then
            FR.ShowProperties()
        Else
            'If cfgDebugLevel >= 2 Then WriteLog("Method SetExchangeParam is starting")
            FR.Password = 30
            FR.ConnectionType = 0
            FR.ComNumber = ComPortNum '0
            FR.BaudRate = BaudRateCode
            FR.Timeout = cfgReadTimeOut
        End If

        'Подключиться к ККМ. Метод Connect открывает COM-порт и проверяет связь с ККМ.
        FR.Connect()
        If FR.ResultCode <> 0 Then
            WriteLog("Connect failed with RC = " & FR.ResultCode & " - " & FR.ResultCodeDescription)
            ' Попытка переподключения
            FR.Disconnect()
            Thread.Sleep(500)
            FR.Connect()
            If FR.ResultCode <> 0 Then
                WriteLog("Reconnect failed with RC = " & FR.ResultCode & " - " & FR.ResultCodeDescription)
                Throw New ApplicationException("Ошибка '" & FR.ResultCodeDescription & "' при подключении к ККМ")
            End If
        End If
        FR.SetExchangeParam()
        If FR.ResultCode = 0 Then
        ...
        FR.GetECRStatus()
        If FR.ResultCode = 0 Then
            WriteLog("ECRMode = " & FR.ECRMode.ToString & " (" & FR.ECRModeDescription & ")")
            WriteLog("ECRAdvancedMode = " & FR.ECRAdvancedMode.ToString)
            'Если режим 8 - есть открытый чек, отменяем его
            'If FR.ECRMode = 8 Then
            '    WriteLog("Found open check, cancelling...")
            '    FR.CancelCheck()
            '    WriteLog("CancelCheck result: " & FR.ResultCode)
            'End If
        ...
        End If
        'Принудительно закрыть смену (для отладки)
    ...
    Public Sub PrintRentalReceipt(ByVal paymentsInfo As System.Collections.Generic.IList(Of DayRentalPaymentInfo)) Implements ITerminalFiscalRegister.PrintRentalReceipt
        ...
        Try
            ...
            If cfgDebugLevel >= 2 Then WriteLog("Method OpenCheck is starting")
            FR.Password = 30
            FR.CheckType = 1 ' 1 = чек продажи

            For Each pi As DayRentalPaymentInfo In paymentsInfo

                'регистрация продажи с вычислением налогов без закрытия чека.
                If cfgDebugLevel >= 2 Then WriteLog("Method FNOperation is starting")
                FR.Password = 30
                FR.Quantity = 1
                FR.Price = pi.PaidSum
                FR.Department = 1
                FR.Tax1 = 1  ' НДС 22% (с 2026)'2
                FR.Tax2 = 0
                FR.Tax3 = 0
                FR.Tax4 = 0
                ' Параметры для ФФД 1.05+
                Try
                    FR.PaymentTypeSign = 4  ' 4 = Полный расчёт
                    FR.PaymentItemSign = 4  ' 4 = Услуга
                Catch
                    ' Свойства могут отсутствовать в старых версиях
                End Try

                If pi.PaidSum < pi.Price Then
                    ServiceName = ServiceName & "/ч" 'было /частично 'ServiceName = ServiceName.Substring(0, 30) & "/частично"
                End If
                If pi.PaidSum > pi.Price Then
                ...
                ' Попытка 1: FNOperation (для ФФД 1.05+)
                If Not registered Then
                    Try
                        WriteLog("Before FNOperation: CheckType=" & FR.CheckType &
                                 ", PaymentTypeSign=" & FR.PaymentTypeSign &
                                 ", PaymentItemSign=" & FR.PaymentItemSign)
                        FR.FNOperation()
                        If FR.ResultCode = 0 Then
                            registered = True
                            WriteLog("FNOperation succeeded")
                        Else
                            WriteLog("FNOperation FAILED: RC=" & FR.ResultCode &
                                     " - " & FR.ResultCodeDescription)
                        End If
                    Catch ex As Exception
                        WriteLog("FNOperation EXCEPTION: " & ex.GetType().Name &
                                 " - " & ex.Message)
                    End Try
                End If

                If Not registered Then
                    Throw New ApplicationException("Не удалось зарегистрировать позицию. Ни один метод не сработал.")
                End If
                TotalPaidSum = TotalPaidSum + pi.PaidSum
            Next pi

            'операция закрытия чека
            If cfgDebugLevel >= 2 Then WriteLog("Method FNCloseCheckEx is starting")
            FR.Password = 30
            ...
            ' Пробуем FNCloseCheckEx, затем CloseCheck
            ...
            Try
                ...
                If FR.ResultCode = 0 Then
                    closed = True
                    WriteLog("FNCloseCheckEx succeeded")
                Else
                    WriteLog("FNCloseCheckEx returned: " & FR.ResultCode & " - " & FR.ResultCodeDescription)
                End If
            Catch ex As Exception
                WriteLog("FNCloseCheckEx not available: " & ex.Message)
            End Try
            ...
            If Not closed Then
                ...
                If FR.ResultCode = 0 Then
                    closed = True
                    WriteLog("CloseCheck succeeded")
                Else
                    WriteLog("CloseCheck failed: " & FR.ResultCode & " - " & FR.ResultCodeDescription)
                    Throw New ApplicationException("Ошибка CloseCheck: " & FR.ResultCodeDescription)
                End If
            End If
...
#Region "Устаревшие методы"
    ...
    Public Sub PrintAdditionalServiceReceiptExOld(ByVal serviceName As String, ByVal servicePrice As Decimal, ByVal paidSum As Decimal, ByVal InfoItems As IList(Of String))
    ...
    Public Sub PrintAdditionalServiceReceiptOld(ByVal serviceName As String, ByVal servicePrice As Decimal, ByVal paidSum As Decimal)
    ...
    Public Sub PrintRentalReceiptOld(ByVal paymentsInfo As System.Collections.Generic.IList(Of DayRentalPaymentInfo))
        ...
        Try
            ...
            For Each pi As DayRentalPaymentInfo In paymentsInfo
                ...
                Else
                    ...
                    Throw New ApplicationException("Ошибка '" & FR.ResultCodeDescription & "' при выполнении метода Sale")
                End If
                TotalPaidSum = TotalPaidSum + 1 'pi.PaidSum
            Next pi
