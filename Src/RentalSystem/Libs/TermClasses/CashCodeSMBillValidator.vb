...
Public Class CashCodeSMBillValidator
    ...
    'В SendCommand между отправкой и чтением — всего 20 мс.
    'Для команды BILL_TYPE (12 байтов) устройству может не хватить времени на обработку.
    'Спецификация CCNET рекомендует минимум 10–20 мс, но реальные устройства CashCode иногда требуют 50–100 мс,
    'особенно при смене режима.
    Private iCmdDelay As Integer = 100 '20
    ...
    Private Function Receive(ByRef Buffer As Byte(), ByVal Length As Int32, Optional ByVal Offset As Int32 = 0) As Boolean
        ...
        Try
            Dim totalRead As Int32 = 0
            While totalRead < Length
                Dim dwBytes As Int32 = BV.Read(Buffer, Offset + totalRead, Length - totalRead)
                If dwBytes = 0 Then Return False
                totalRead += dwBytes
            End While
            If cfgDebugLevel >= 2 Then WriteLog("< " & BitConverter.ToString(Buffer, Offset, Length))
            Return True
        Catch ex As TimeoutException
            WriteLog("Receive timeout")
            Return False
        Catch ex As Exception
    ...
    Private Function Transmit(ByRef Cmd As Byte(), ByRef Rsp As Byte()) As Integer
        ...
        For i As Integer = 0 To 2
            ...
            Else 'ошибка передачи
                'если не TIMEOUT, то послать NAK
                If iErr <> RE_TIMEOUT Then
                    cmdACK(3) = NAK
                    TransmitCMD(cmdACK, Nothing)
                End If
                'Всегда ждать перед повтором, в том числе при TIMEOUT,
                'чтобы устройство успело завершить передачу и DiscardInBuffer
                'в следующем SendCommand корректно очистил буфер
                If iCmdDelay Then Thread.Sleep(iCmdDelay)
            End If
        Next i
    ...
    Private Function TransmitOld(ByRef Cmd As Byte(), ByRef Rsp As Byte()) As Integer
        'Complete protocol exchange
        '	1) Sending the frame and receiving a response using TransmitCMD
        '	2) Checking the device response and determining whether ACK or NAK should be sent 
        '	3) Sending ACK or NAK message to the device or retransmitting the command up to 3 times 
    ...
    Public Sub Initialize() Implements ITerminalBillValidator.Initialize
        ...
        Try
            ...
            Else
                ...
                'Пауза после открытия порта, чтобы устройство стабилизировалось
                Thread.Sleep(150)
