    ...
    Public Function AcceptBarCode(ByRef Break As Boolean) As String Implements ITerminalBarCodeScanner.AcceptBarCode
        ...
        Try
            ...
            If Not BCS.IsOpen Then
                ...
                Throw New ApplicationException("Не удалось открыть " & BCS.PortName)
                Exit Function
            Else
                ...
                'Дождаться окончания приёма штрих-кода, периодически проверяя, не прервана ли операция пользователем
                Do
                    Try
                        synBuffer.AcquireReaderLock(10000)
                    Catch ex As ApplicationException
                        WriteLog("The reader lock request timed out")
                        Throw New ApplicationException("Не удалось получить требуемый ресурс")
                        Exit Function
                    End Try
                    Try
                    ...
                    If BarCode <> "" Then 'что-то принято
                        WriteLog("< " & BarCode)
                        Exit Do
                    End If
        ...
        Catch ex As Exception
            ...
            Throw New ApplicationException("Аварийное завершение чтения штрих-кода", ex)
        Finally
            If BCS.IsOpen Then
                BCS.DiscardInBuffer()
                BCS.DiscardOutBuffer()
